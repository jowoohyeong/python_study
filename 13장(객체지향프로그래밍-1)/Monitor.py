...
class Monitor:
    # 필드 선언
    # __ private 성질은 같은 클래스에서만 접근 가능
    __company = ""
    __inch = 0
    __price = 0
    __color = ""
    ...
    # 오버로딩 : 영어 뜻으로는 과적하다,
    # 같은 메서드명으로 다른 일을 하게끔 만드는 작업
    # 매개변수의 타입과 개수에 따라서 같은 이름의 메서드라도 다른 메서드가 호출이 되는 형태를 지칭
    # def __init__(self):
    #    print("기본생성자 호출")

    # 매개변수가 4개 존재하는 생성자
    def __init__(self, company, inch, price, color):
        # self.company 는 멤버변수 칭하는 것, company 은 외부에서 생성자를 호출할 때 매개변수값으로 들어오는 것을 의미
